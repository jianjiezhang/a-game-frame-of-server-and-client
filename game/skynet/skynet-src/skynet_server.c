...
static const char*
cmd_getname(struct skynet_context *context, const char * param){
	uint32_t handle = tohandle(context, param);
	char *name = skynet_handle_getname(handle);
	return name;
}
...
static const char*
cmd_getvenv(struct skynet_context *context, const char *param)
{
	return skynet_getvenv(param);
}
static const char*
cmd_setvenv(struct skynet_context *context, const char *param)
{
	size_t sz = strlen(param);
	char key[sz + 1];
	int i;
	...
	bool ok = skynet_setvenv(key, param);
	const char* result = ok ? "1":"0";
	return result;
}
