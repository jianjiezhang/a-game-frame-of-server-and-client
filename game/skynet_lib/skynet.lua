...
function skynet.catch(func, ...)
	local ok, err = xpcall(func, debug.traceback, ...)
	if not ok then
		err = tostring(err)
		skynet.error("skynet.catch error:", err)
	end
	return ok
end
...
function skynet.getname(handle)
	return c.command("getname",skynet.address(handle))
end
...
function skynet.exit_service()
	local stop = _G[SERVICE_NAME].stop
	if stop then
		stop()
	end
	skynet.exit()
...
function skynet.getvenv(key)
	return (c.command("GETVENV", key))
end
function skynet.setvenv(key, value)
	c.command("SETVENV", key .. " " .. value)
end
...
function skynet.check_async_call(session)
	if not acall_sessions[session] then
		return
	end
	local handle = acall_sessions[session][2]
	skynet.wakeup(handle)
end
function skynet.async_call(addr, typename, ...)
	local reqobj = table.pack(addr, typename, ...)
	local req = skynet.request(reqobj)
	local timeout = 800
	for r, resp in req:select(timeout) do
		req:close()
		if resp then
			return table.unpack(resp, 1, resp.n)
		else
			return nil, string.format("async_clal failed (%s)", skynet.address(addr))
		end
	end
	req:close()
	return nil, string.format("async_call timeout (%s)", skynet.address(addr))
end
...
function skynet.dispatch(typename, func, module)
	local p = proto[typename]
	if func then
...
function skynet.dispatch_message(...)
	...
	if not succ then
		--err = skynet.var_dump(args) .. "\n" .. tostring(err)
		skynet.error("dispatch_message error!message:, error:", skynet.vardump(...), tostring(err))
	end
	--assert(succ, err)
end

local function handle_proto(address, proto, result, ...)
	if result == nil then
		return
	end
	if result == false then
		local err_args = {...}
		return skynet.send(address, "lua", 0,Proto.newf(proto.__name, "errno", 4, "err_args", err_args))
	end
	local rd = Proto.newf(proto.__name, ...)
	--rd = Proto.pack(rd)

	return skynet.send(address,"lua", 0, rd)
end
local function handle_message2(call,address, proto, result,...)
	if call and result ~= nil then
		skynet.retpack(result, ...)
		return
	end
	if proto then
		handle_proto(address, proto, result, ...)
	end
end
local function handle_message(p,call, source, module, method,...)
	if p.module then
		if type(module) == "table" and module.__name then
			handle_message2(call, source, module, p.module[module.__name](module))
			return
		end
		handle_message2(call, source, nil, p.module[module](method, ...))
		return
	end
	if type(method) == "table" and method.__name then
		handle_message2(call, source, method, _G[module][method.__name](method))
		return
	end
	handle_message2(call, source, nil, _G[module][method](...))
end

function skynet.dispatch_lua(...)
	local ok = skynet.catch(skynet.dispatch_lua2, ...)
	if not ok then
		skynet.error("dispatch_lua error:", skynet.vardump(...))
	end
end
function skynet.dispatch_lua2(session, source, module, method, ...)
	if module == 0 then
		role.send_proto(method)
		return
	end
	handle_message(proto[skynet.PTYPE_LUA], session ~= 0, source, module, method, ...)
end
function skynet.dispatch_system(...)
	local ok = skynet.catch(skynet.dispatch_system2,...)
	if not ok then
		skynet.error("dispatch_system error:", skynet.vardump(...))
	end
end
function skynet.dispatch_system2(session, source, module, method, ...)
	handle_message2(session ~= 0, source, nil, _G[module][method](...))
end


function skynet.timer_cmp(e1, e2)
	return e1.value < e2.value
end
function skynet.timer_init()
	__timers = minheap:new(skynet.timer_cmp)
end
function skynet.timer_loop()
	...
	while true do
		...
		__timers:pop()
		skynet.pcall(root_func, root_args)
	end
	while true do
		local min_time = skynet.get_min_time()
		if not min_time or min_time > now then
			break
		end
		table.remove(__timeouts)
	end
end
function skynet.timer_root_time()
	local _, _, time = __timers:peek()
	return time
end
function skynet.get_min_time()
	return next(__timeouts) and __timeouts[#__timeouts]
end
function skynet.timer_check()
	if not __timers then
		return
	end
	local _, _, min_time = __timers:peek()
	if min_time then
		local min_timeout = skynet.get_min_time()
		if not min_timeout or min_time < min_timeout then
			local intvl = min_time - stdin.time()
			skynet.timeout(intvl <= 0 and 0 or intvl , skynet.timers_dispatch)
			table.insert(__timeouts, min_time)
		end
	end
end
function skynet.timers_dispatch()
	if not __timers then
		return
	end
...
function skynet.timer_push_ends(func, ends, args)
	__timers:push(func, args, ends)
	skynet.timer_check()
end
function skynet.timer_remove(func, args)
...
function skynet.error(format,...)
	local prelog = "[" .. os.date("%F %T", os.time()) .."]"
	local log_func = _G[SERVICE_NAME].GETLOG
	if log_func then
		prelog = prelog .. log_func()
	end
	local log = prelog .. format
	c.error(log, ...)
end

function skynet.warn(format, ...)
	local prelog = "[" .. os.date("%F %T", os.time()) .."]"
	local log_func = _G[SERVICE_NAME].GETLOG
	if log_func then
		prelog = prelog .. log_func()
...
do
	...
	REG {
		...
		unpack = skynet.unpack,
		pack = skynet.pack,
		dispatch = skynet.dispatch_system
	}
end

function skynet.init(...)
	local init = _G[SERVICE_NAME].init
	if not init then
		return true
	end
	skynet.error("init begs...." .. SERVICE_NAME)
	local ok = skynet.catch(init, ...)
	--local ok = init(...)
	if not ok then
		skynet.send(".launcher", "lua", "ERROR", skynet.self())
		skynet.error("init failed...." .. SERVICE_NAME)
		skynet.exit()
	else
		skynet.send(".launcher", "lua", "LAUNCHOK", skynet.self())
		collectgarbage("collect")
		skynet.error("init succ...." .. SERVICE_NAME)
	end	
end

function skynet.stop(...)
	skynet.error("stop ...")
	local stop = _G[SERVICE_NAME].stop
	if stop then
		stop(...)
...
function skynet.vardump_(value, pretty, maxDepth, indent, visited)
	indent = indent or 0
	...
	elseif t == "table" then
		...
		for k, v in pairs(value) do
			...
			if type(k) == "string" and string.match(k, "^[%a_][%w_]*$") then
				...
				keyStr = "[" .. skynet.vardump_(k, pretty, maxDepth, indent+1, visited) .. "]"
			end

			local valueStr = skynet.vardump_(v, pretty, maxDepth, indent + 1, visited)
...
function skynet.vardump(...)
	local str = ""
    for i = 1, select('#', ...) do
        local val = select(i, ...)
        str = str .. skynet.vardump_(val) .. ","
    end
    return str
end
function skynet.vardump2(...)
	local str = ""
    for i = 1, select('#', ...) do
        local val = select(i, ...)
        str = str .. skynet.vardump_(val, true) .. ","
    end
    return str
end
function skynet.copy(obj, seen)
...
function skynet.send_role_proto(role_id, proto)
	local procname = skynet.role_procname(role_id)
	skynet.send(procname, "lua", 0, proto)
end
function skynet.send_pid_proto(pid, proto)
	if pid then
		skynet.send(pid, "lua", 0, proto)
	end
end
function skynet.send_proto(proto)
	if not role_data.is_connected() then
		return
	end
	local gatepid = role_data.get_gatepid()
	skynet.error("send:", skynet.vardump(proto))
	skynet.send(gatepid, "lua", "gate", "send_proto", role_data.get_role_id(), proto)
end
function skynet.send_protofe(name, ...)
	local proto = Proto.newf(name, ...)
	skynet.send_proto(proto)
end

function skynet.server_id()
	return skynet.getenv("server_id") or 39
end
function skynet.role_id(seq_id)
	return skynet.server_id()
end


function skynet.is_conf(name)
	if string.sub(name, 1, 2) == "f_" or string.sub(name, 1, 2) == "k_" then
		return true
	end
	return false
end
function skynet.searchpath(name)
	local err = {}
...
function skynet.processes()
	return skynet.call(".launcher", "lua", "LIST")
end


local function assoc_upvalue(upvalues, name, addr, id, obj, i)
	local upvalue = upvalues[name]
	if not upvalue then
		upvalue = {}
		upvalues[name] = upvalue
	end
	if not upvalue[addr] then
		upvalue[addr] = obj
		upvalue[id] = i
	end
end
local function hold_function(module, obj, upvalues, updated_funcs)
	updated_funcs[obj] = true
	for i = 1, math.huge do
		local name, value = debug.getupvalue(obj, i)
		if not name then
			break
		end
		assoc_upvalue(upvalues, name, "old_addr", "old_id", obj, i)
		if type(value) == "function" then
			if not updated_funcs[value] then
				hold_function(name, value, upvalues, updated_funcs)
			end
		elseif type(value) == "table" and string.upper(name) == name and name ~= "_ENV" then
			for k, v in pairs(value) do
				if type(v) == "function" and not updated_funcs[v] then
					hold_function(module, v, upvalues, updated_funcs)
				end
			end
		end
	end
end
local function find_upvalue_byname(obj, name)
	for i = 1, math.huge do
		local k, value = debug.getupvalue(obj, i)
		if not k then
			return
		end
		if k == name then
...
local function hotfix_function(module, old_obj, new_obj, upvalues, mfuncs, updated_funcs)
	if old_obj then
		mfuncs[old_obj] = new_obj
	end

	updated_funcs[new_obj] = true
	for i = 1, math.huge do
		local name, value = debug.getupvalue(new_obj, i)
		if not name then
			break
		end
		...
		if type(value) == "function" then
			if not updated_funcs[value] then
				local ovalue = old_obj and find_upvalue_byname(old_obj, name)
				hotfix_function(module, ovalue, value, upvalues, mfuncs, updated_funcs)
			end
		elseif type(value) == "table" and string.upper(name) == name and name ~= "_ENV" then
			for k, v in pairs(value) do
				if type(v) == "function" and not updated_funcs[v] then
					hotfix_function(module, nil, v, upvalues, mfuncs, updated_funcs)
				end
			end
		else
			assoc_upvalue(upvalues, name, "new_addr", "new_id", new_obj, i)
		end
	end
end

local function hotfix_table(module, old_obj, new_obj, upvalues, mfuncs, updated_funcs)
	for k, v in pairs(old_obj) do
	...
	for k, v in pairs(new_obj) do
		local ov = old_obj[k]
		local ot, nt = type(ov), type(v)

		if not ov then
			old_obj[k] = v
			if nt == "function" then
				hotfix_function(module, ov, v, upvalues, mfuncs, updated_funcs)
			end
		elseif nt == ot and nt == "function" then
			old_obj[k] = v
			hotfix_function(module, ov, v, upvalues, mfuncs, updated_funcs)
		end
	end
...
local function replace_upvalues(upvalues)
	for k, v in pairs(upvalues) do
		if v.new_addr and v.old_addr and (type(v) ~= "table" or string.upper(k) ~= k) then
			local name, value = debug.getupvalue(v.old_addr, v.old_id)
			debug.setupvalue(v.new_addr, v.new_id, value)
		end
	end
end
function skynet.hotup(name)
	local old_obj = package.loaded[name]
	if not old_obj then
		return
	end
	local filename, f = skynet.searchpath(name)
	if not filename then 
		skynet.error("hotup failed:", skynet.vardump(f))
		return
	end

	local ok, new_obj = pcall(f)
	if not ok then
		skynet.error("pcall not ok")
		return
	end

	local upvalues, mfuncs, updated_funcs = {}, {}, {}
	hotfix_table(name, old_obj, new_obj, upvalues, mfuncs, updated_funcs)
	_G[name] = old_obj

	replace_upvalues(upvalues)
	return mfuncs
end

function skynet.role_procname(role_id)
