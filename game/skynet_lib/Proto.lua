...
Proto.m_scene_enter_tos = {__name = "m_scene_enter_tos", __mod = 3, error = 0, id = 0}
...
function Proto.newf(name, ...)
    name = Proto.fname(name)
    local proto = Proto.new(name, ...)
    return proto
end

function Proto.fname(name)
    return name:sub(1, -2) .. "c"
end
