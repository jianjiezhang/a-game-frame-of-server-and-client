...
socket_message[1] = function(id, size, data)
	--skynet.error("socket message:DATA:" .. id)
...
socket_message[2] = function(id, ud , addr)
	--skynet.error("socket message:CONNECT:" .. id)
	local s = socket_pool[id]
	if s == nil then
		return
	end
...
socket_message[3] = function(id)
	--skynet.error("socket message:CLOSE:" .. id)
	local s = socket_pool[id]
	if s then
		s.connected = false
		wakeup(s)
...
socket_message[4] = function(id, newid, addr)
	--skynet.error("socket message:ACCEPT:" .. id)
	local s = socket_pool[id]
	if s == nil then
		driver.close(newid)
		return
...
socket_message[5] = function(id, _, err)
	--skynet.error("socket message:ERRPR:" .. id)
	local s = socket_pool[id]
	if s == nil then
		driver.shutdown(id)
...
socket_message[6] = function(id, size, data, address)
	--skynet.error("socket message:UDP:" .. id)
	local s = socket_pool[id]
...
socket_message[7] = function(id, size)
	skynet.error("socket message:warning:" .. id)
	local s = socket_pool[id]
	if s then
...
function socket.read(id, sz)
	...
	if s.closing or not s.connected then
		skynet.error("closing and not coonneted")
...
--packet
function socket.read_packet(fd)
    local len_buf = socket.read(fd, 2)
    if not len_buf then
		skynet.error("read len failed")
        return
    end
    local len = string.unpack(">I2", len_buf)
    local data = socket.read(fd, len)
    return data
end
function socket.send_packet(fd, data)
    local len = #data
    local pack = string.pack(">I2", len) .. data
    socket.write(fd, pack)
end

function socket.recv_proto(fd)
    local data = socket.read_packet(fd)
    if not data then
		skynet.error("read data failed")
        return false
    end
    --解密+协议
    local proto = Proto.unpack(data)
    skynet.error("recv" .. "[" .. tostring(fd) .. "]:", skynet.vardump(proto))
    return proto
end

function socket.send_proto(fd, proto)
    local data = Proto.pack(proto)
    socket.send_packet(fd, data)
end
