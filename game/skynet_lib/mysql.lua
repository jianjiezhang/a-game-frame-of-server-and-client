...
local function _recv_field_packet(self, sock)
    ...
    if not packet then
        skynet.error("err:",skynet.vardump(err))
        return nil, err
    end
...
function mysql.disconnect(self)
    self.sockchannel:close()
    setmetatable(self, nil)
end

function mysql.query(self, query)
...
-- 注册预处理语句
function mysql.prepare(self, sql)
...
--重置预处理句柄
function mysql.stmt_reset(self, stmt)
...
--关闭预处理句柄
function mysql.stmt_close(self, stmt)
